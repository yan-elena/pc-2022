...
import common.Timer;
import common.TimerEvent;
import io.vertx.core.json.JsonObject;
import smart_room.Event;
import smart_room.LightDevice;
import smart_room.distributed.*;

public class LightControllerAgent extends BasicEventLoopAgent {
	...
	public LightControllerAgent(
				LightDevice ld, double threshold) throws Exception {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			LightControllerAgent agent = new LightControllerAgent(ld, 0.2);
			agent.start();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
